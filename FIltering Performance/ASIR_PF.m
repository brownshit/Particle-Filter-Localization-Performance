function...
    [x_esti,pt_pres,w_pres] = ASIR_PF(pt_prev,w_prev,z,dt,measu_var,abs_process_n,E_w,Npt,prev_pt_vel)
    %{
    ...
    %       Prediction
    for p = 1:Npt
        pt_Aux(:,p) = fx(pt_prev(:,p),dt,prev_pt_vel(:,p),E_w)+...
    ...
    for p = 1:Npt
        err_mu = z-hx(pt_Aux(:,p));
        likelihood = exp((-1/(2*measu_var))*(err_mu'*err_mu))./((sqrt(2*pi))^size(z,1)*std);
        w_Aux(p) = w_prev(p)*likelihood;    %weight revision
    ...
    for q = 1:Npt
        pt_pres(:,q) = ...
            fx(pt_prev(:,...
            ind(q,1)),...
            dt,prev_pt_vel(:,ind(q,1)),E_w)+...
            [abs_process_n(1,1)*randn;...
            abs_process_n(2,1)*randn];

        err_mu = z-hx(pt_Aux(:,ind(q,1)));
        err_pres = z-hx(pt_pres(:,q));

        likelihood_mu = exp((-1/(2*measu_var))*(err_mu'*err_mu))./((sqrt(2*pi))^size(z,1)*std);
        likelihood = exp((-1/(2*measu_var))*(err_pres'*err_pres))./((sqrt(2*pi))^size(z,1)*std);
        
        w_pres(q) = likelihood/likelihood_mu;    %weight revision
    end
end